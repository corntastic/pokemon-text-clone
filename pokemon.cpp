...
Arrays:
		...
		pokestat: pokemon [base stats of HP ATK DEF SA SD SPD, evolve level, type1, type2]
		moveset: [pokemon][level]
		movestats [move#] [type, power, acc]
		map [x] [y]
		typechart [attacking type] [def type] norm fire water ele grass ice fight psn grnd fly psy bug rck ghost drg default
...
void loadfile();
void savefile();
int trainerBattle(string job, string name, int templevel, int levelMod, int SLMod, int pokemon0, int pokemon1=0, int pokemon2=0, int pokemon3=0, int pokemon4=0, int pokemon5=0);
void gamestart();
...
void turnResult(int& eHP, int& pHP, int epoke, int ppoke, int elvl, int plvl, int emove, int pmove);
void xp(double trainer, int epoke, int elvl, int ppoke, int& pexp, int& plvl, int partypos);
...
int main() 
{
	...
	while (menu !='q') // q is quit command, checks events per area, then presents the menu
	{
		cout << "\t\t\t\t\n" << map[x][y] << "\n\n";
		event(x,y,evFlags);
		menu = travel(x, y);
	...
	while(1)
	{
		...
		switch (action)
		{
				...
				if (bounds(x,y,'w') == 1) // checks if up is within bounds
				{
					...
					if (y==0) // double checking so you cant go around the world
					{
						...
						if (battleChance < 7) // checks for trainer battle
							if (trainerBattle(trainers[rand()%30][0], trainers[rand()%30][1], maplvl[x][y], 3, 0, trainerpokemon(maplvl[x][y]), trainerpokemon(maplvl[x][y]), trainerpokemon(maplvl[x][y])))
							{
								y--;
								return 'w';
							}
							else // else sends you to last town healed at
								{
									...
									cout << "Nurse Joy: All pokemon fully restored! We hope to see you again!"<< endl;
									x = lastx;
									y = lasty;
									return 'w';						
								}					
						else if ( battleChance < 55) // checks if its a random battle
							if (wildBattle(maplvl[x][y], 3, wildpoke(x,y)) == 1) // checks if you win the wild battle
							{
								y--;
								return 'w';
							}
			...
			case 's': // move down
				...
				{
					if ((x==2 && y== 7) || (x==6 && y== 8)) // if you need to surf from pallet or below fuschia
					{
					...
					else // if its not a surfable area
					{
						if (city(x,y) == 0) // checks if its not a city
							if (battleChance < 7) // checks for trainer battle
								...
								if (wildBattle(maplvl[x][y], 3, wildpoke(x,y)) == 1) // if battle won, move down
									{
										y++;
										return 's';
									}
				...
				if (bounds(x,y,'d') == 1) // checks if in bounds
				{
					if (x==9) // checks its not looping
						...
						if (battleChance < 7) // checks for trainer battle
							...
							if (wildBattle(maplvl[x][y], 3, wildpoke(x,y)) == 1) // if player wins random battle
								{
									x++; // move right
									return 'd';
								}
					...
					else // not viridian city
					{
						if (city(x,y) == 0) // checks if its not a city
							if (battleChance < 7) // checks for trainer battle
								...
								if (wildBattle(maplvl[x][y], 3, wildpoke(x,y)) == 1) // if they win a wild battle
									{
										x--; // move left
										return 'a';	
									}
			...
			case 'h': // help
				cout << "wasd\t- move cardinally \nt\t- fight trainers (outside of cities) \nc\t- fight wild pokemon (outside of cities) \ng\t- challenge the Gym (only in certain cities) \np\t- heal at the pokemon center (only in cities) \nb\t- access the box (only in cities) \nP\t- organize the party (capitals) \ng\t- challenge the gym (only in cities) \nS\t- save (capitals)\nq\t- save and quit" << endl;
				continue;
			...
			case 't': //trainer battle, back to last pokecenter if loss
				if (!city(x,y))
				{
					...
					win = trainerBattle(trainers[rand()%30][0], trainers[rand()%30][1], maplvl[x][y], 3, 0, trainerpokemon(maplvl[x][y]), trainerpokemon(maplvl[x][y]), trainerpokemon(maplvl[x][y]));
			...
			case 0: // indigo plateau, vic road, 23, 24 contd
			{
				switch(y)
				{
					case 2: // indigo plateau
						{
							cout << "Guard: Welcome to the Pokemon League. \nGuard: Trainers who have collected 8 badges have the opportunity to challenge the Elite Four! \nGuard: You possess 8 badges, please proceed!" << endl;
							cout << "Advisor: Remember, you can call Nurse Joy to heal your pokemon only twice during the Elite 4 Gauntlet!" << endl;
							break;
						} // end y case 2
					case 3: // vic road
						{
					...
					case 5: // route 24
						{
							L=1;
							cout << "Gary: What? " << endl
								 << "Gary: " << pName << "! What a surprise to see you here!" << endl
								 << "Gary: So, you're going to the Pokemon League? " << endl
								 << "Gary: You collected all the gym badges too? That's cool!" << endl
								 << "Gary: Then I'll whip you, " << pName << ", as a warmup for the Pokemon League!" << endl
								 << "Gary: Come on!" << endl;
							switch (garystarter)
								{
									case 1:
										win = trainerBattle("Gary", "Oak", 45, 0, 1, 18, 65, 111, 58, 130, 3);
										break;
									case 4:
										win = trainerBattle("Champion", "Gary", 45, 0, 1, 18, 65, 111, 130, 102, 6);
										break;
									case 7:
										win = trainerBattle("Champion", "Gary", 45, 0, 1, 18, 65, 111, 102, 58, 9);
										break;
								}	
							...
							if (win == 0)
								{
									cout << "Gary: Hahaha!" << endl
										 ...
										 << "Gary: Go train some more, you loser!" << endl << endl;
									for (int i = 0; i < 6; i++)
										party[i][2] = stat("HP", party[i][0], party[i][1]);
									cout << "Nurse Joy: All pokemon fully restored! We hope to see you again!"<< endl;
									flag[x][y] = -1;
							...
							cout << "Gary: That loosened me up! I'm ready for the Pokemon League!" << endl
								 << "Gary: " << pName << ", you need more practice." << endl
								 << "Gary: But hey, you know that! I'm out of here, smell ya!" << endl;
							flag[x][y] = 1;	
							break;
						} // end y case 5
				} // end switch y
...
int bounds(int x, int y, char d) // checks whether movment request is out of bounds
{
	ifstream maploader;
	int i, j;
...
void gym(int x, int y) // gym script, if through the gauntlet, award badge and increment total
{ 
	int win = 1;
	int joy = 2;
	char choice;
	switch (y)
	{
		case 2:
			if (x == 0) // e4
			{
				...
				if (win == 1)
				{
					...
					cout << "Lorelei: Welcome to the Pokemon League! \n"
							...
							<< "Lorelei: Your pokemon will be at my mercy when they're frozen solid! \n"
							<< "Lorelei: Hahaha! Are you ready? " << endl;
					if (trainerBattle("Elite 4", "Lorelei", 52, 0, 1, 87, 91, 80, 124, 131))
					{
							cout << "Lorelei: How dare you! \n"
								<< "Lorelei: You're better than I thought! Go on ahead! \n"
								<< "Lorelei: You only got a taste of the Pokemon League's power!" << endl;
						if (joy > 0) // call nurse joy
						{
							...
							if (choice == 'p')
								{			
									for (int i = 0; i < 6; i++)
										party[i][2] = stat("HP", party[i][0], party[i][1]);
									cout << "Nurse Joy: All pokemon fully restored! Thank you for using the Pokemon Leaguebulance!"<< endl;
									joy--;
								}
						}
					}
				...
				if (win == 1)
				{
					...
					cout << "Bruno: I am Bruno of the Elite 4!\n"
						<< "Bruno: Through rigorous training, people and pokemon can become stronger! \n"
						<<"Bruno: I've weight trained with my pokemon! \n"
						<< "Bruno: " << pName << "!\n"
						<< "Bruno: We will grind you down with our superior power! \n"
						<< "Bruno: Hoo hah!" << endl;
						
					if (trainerBattle("Elite 4", "Bruno", 54, 0, 1, 95, 107, 106, 95, 68))
					{
						cout << "Bruno: Why? How could I lose? \n"
							<< "Bruno: My job is done! Go face your next challenge!" << endl;
				...
				if (win == 1)
				{
					...
					cout << "Agatha: I am Agatha of the Elite 4,\n"
						...
						<< "Agatha: That old duff was once tough and handsome! That was decades ago! \n"
						<< "Agatha: Now he just wants to fiddle with his pokedex! He's wrong!\n"
						<< "Agatha: Pokemon are for fighting! \n"
						<< "Agatha: " << pName << ", I'll show you how a real trainer fights!" << endl;
						
					if(trainerBattle("Elite 4", "Agatha", 55, 0, 1, 94, 42, 93, 24, 94))
					{
						cout << "Agatha: Oh ho! You're something special, child! \n"
							<< "Agatha: You win! I see what the old duff sees in you now! \n"
							<< "Agatha: I have nothing else to say. Run along now, child!" << endl;
				...
				if (win == 1)
				{
					...
					cout << "Lance: Ah! I heard about you, " << pName << "!\n"
						<< "Lance: I lead the Elite 4. You can call me Lance, the dragon trainer! \n"
						...
						<< "Lance: Your league challenge ends with me, " << pName << "!" << endl;
					
					if (trainerBattle("Elite 4", "Lance", 58, 0, 1, 130, 148, 148, 142, 149))
					{
						cout << "Lance: That's it!\n"
							 << "Lance: I hate to admit it, but you are the pokemon master!\n"
							 << "Lance: I still can't believe my dragons lost to you, " << pName << "!\n"
							 << "Lance: You are now the Pokemon League Champion!\n"
							 << "Lance: . . . Or you would have been, but you have one more challenge ahead. \n"
				...
				{
					// gary
					// cin = trainerBattle("Champion", "Gary", 52, 0, 1, 87, 91, 80, 124, 131, int pokemon5)
					
					cout << "Gary: Hey! \n"
						 << "Gary: I was looking forward to seeing you, " << pName << "!\n"
						 << "Gary: My rival should be strong to keep me sharp! \n"
						 << "Gary: While working on my pokedex, I looked all over for powerful pokemon. \n"
						 << "Gary: Not only that, I assembled teams that could beat any pokemon type! \n"
						 << "Gary: And now!\n"
					...
					switch (garystarter)
						{
							case 1:
								win = trainerBattle("Champion", "Gary", 60, 0, 1, 18, 65, 112, 59, 130, 3);
								break;
							case 4:
								win = trainerBattle("Champion", "Gary", 60, 0, 1, 18, 65, 112, 130, 103, 6);
								break;
							case 7:
								win = trainerBattle("Champion", "Gary", 60, 0, 1, 18, 65, 112, 103, 59, 9);
								break;
						}
					...
					if (win == 0)
					{
						cout << "Gary: Hahaha! I won, I won! \n"
							 ...
							 << "Gary: You did well to even reach me, Gary, the pokemon genius! \n"
				...
				else // win against gary
				{
					...
					cout << "Gary: NO!\n"
						 ...
						 << "Gary: Although I don't like to admit it. . ." << endl << endl;
						 
					cout << "Oak: " << pName << "! \n"
						 << "Oak: So, you won! Congratulations! You're the new Pokemon League Champion!\n"
						 << "Oak: You've grown up so much since you first left with " << ((garystarter == 1) ? pokedexname[7] : pokedexname[garystarter-3]) << "!\n"
						 ...
						 << "Oak: You have forgotten to treat your pokemon with trust and love! \n"
						 << "Oak: Without them, you will never become the Champion again! \n"
						 << "Oak: " << pName << "!\n"
						 << "Oak: You understand that your victory was not just your own doing! \n"
						 << "Oak: The bond you share with your pokemon is marvelous! \n"
						 << "Oak: " << pName << ", come with me! \n"
						 << "\nProfessor Oak leads you beyond the door, into a dark room. \n\n"
						 << "Oak: Er-hem! Congratulations, " << pName << "!\n"
						 << "Oak: This floor is the Pokemon Hall of Fame! \n"
						 << "Oak: Pokemon League Champions are honoured for their exploits here. \n"
						 << "Oak: Their pokemon are also recorded in the Hall of Fame. \n"
						 << "Oak: " << pName << ", You have endeavoured hard to become the new League Champion! \n"
						 << "Oak: Congratulations, " << pName << ", you and your pokemon are Hall of Famers!\n" << endl;
						 ...
						 hall << pName << '\t' << dt << endl;
						 for (int i = 0; i < 6; i++)
						 	hall << '\t' << pokedexname[party[i][0]] << '\t' << party[i][1] << endl;
						hall << endl;
			...
			else if (x==2 && badge[1] != 1) // pewter
			{
				...
				if (win == 1)
				{
					...
					if (win == 1)
						cout << "Disciple: Wait, lightyears measures distance, not time!" << endl << endl;
				}
					
				
				if (win == 1)
				{
					cout << "Brock: I'm Brock! I'm Pewter's Gym leaeder! \nBrock: I believe in rock-hard defense and determination!\nBrock: That's why all my pokemon are rock-type!\nBrock: Do you still want to challenge me? Fine then, show me your best!\n" << endl;
					if 	(trainerBattle("Leader", "Brock", 12, 0, 2, 74, 95))
					{	
						cout << "Brock:I took you for granted.\nBrock: As proof of your victory, here's the Boulder Badge!" << endl
								<< pName << " received the Boulder Badge!" << endl 
								<< "Brock: That's an official Pokemon League badge. It's bearer becomes more powerful." <<
								"Brock: There are all kinds of trainers in the world.\nBrock: You appear to be very gifted as a pokemon trianer. \nBrock: Go to the gym in Cerulean City and test your abilities!\n" << endl;
						badge[1] = 1;
						badge[0] += 1;
						break;	
					}
			...
			else if(x==7 && badge[2] != 1) // cerulean
			{
				...
				if (win == 1)
				{
					cout << "Disciple: Misty is training right now, don't disturb her!" << endl;
					win = trainerBattle("Gym", "Disciple", 16, 0, 1, 116);
					if (win == 1)
						cout << "Disciple: Sorry Misty!" << endl << endl;
				}				
				
				if (win ==1 )
				{
					cout << "Misty: Hi, you're a new face!\nMisty: Trainers who want to turn pro have to have a policy about pokemon.\nMisty: My policy is an all-out offensive with water-type pokemon!\n" << endl;
					if 	(trainerBattle("Leader", "Misty", 18, 0, 3, 120, 121))
					{
						cout << "Misty: Wow! You're too much!\nMisty: All right!\nMisty: You can have the Cascade Badge to show you beat me!" << endl;
		...
		case 4:
			if (x==5 && badge[4] != 1) // celadon
			{
				if (win == 1)
				{
					...
					if (win == 1)
						cout << "Disciple: Don't move, I'm going to call the cops!" << endl << endl;
				}	
				...
				if (win == 1)
				{
					cout << "Erika: Hello. Lovely weather, isn't it? It's so pleasant.\nErika: . . . Oh dear. . . I must have dozed off. Welcome\nErika: My name is Erika. I am the leader of the Celadon Gym.\nErika: I teach the art of flower arranging. My pokemon are of the grass type.\nErika: Oh, I'm sorry. I had no idea that you wished to challenge me.\nErika: Very well, but I shall not lose!" << endl;
					
					if 	(trainerBattle("Leader", "Erika", 24, 0, 2, 114, 71, 45))
					{
						cout << "Erika: Oh! I concede defeat\nErika: You are remarkably strong.\nErika: I must confer to you the Rainbow Badge." << endl;
						cout << pName << " recieved the Rainbow Badge! " << endl;
						cout << "Erika: You are cataloging pokemon? I must say I'm impressed.\n Erika: I would never collect pokemon if they were unattractive. \n" << endl;
						badge[4] = 1;
						badge[0] += 1;
						break;	
					}
			...
			else if(x==7 && badge[6] != 1) // saffron
			{
				...
				if (win == 1)
				{
					...
					if (win == 1)
						cout << "Disciple: You can't escape! Once she catches you. . .!" << endl;
				}
				...
				if (win == 1)
				{
					cout << "Sabrina: I had a vision of your arrival!\nSabrina: I have had psychic powers ever since I was a child. \nSabrina: I first learned to bend spoons with my mind. \nSabrina: I dislike fighting, but if you wish, I will show you my powers!\n" << endl;
					if 	(trainerBattle("Leader", "Sabrina", 38, 0, 2, 64, 122, 49, 65))
					{
						cout << "Sabrina: I'm shocked! But, a loss is a loss. \nSabrina: I admit, I didn't work hard enough to win! \nSabrina: You earned the Marsh Badge!" << endl;
						cout << pName << " received the Marsh Badge!" << endl;
						cout << "Sabrina: Everyone has psychic powers. Some people just don't realize it yet! \n" << endl;
						badge[6] = 1;
						badge[0] += 1;
						break;	
					}
		...
		case 5:
			if (x==2 && badge[0]==7 && badge[8] != 1) // virid
			{
				...
				if (win == 1)
				{
					cout << "Disciple: I bet you're dying to know who our boss is!" << endl;
					win = trainerBattle("Gym", "Disciple", 34, 0, 1, 104, 104);
					if (win == 1)
						cout << "Disciple: I'm sorry G--hnggg!" << endl;
				}
				...
				if (win == 1)
				{
					cout << "Disciple: You can't fight our leader till you get past me!" << endl;
				...
				if (win == 1)
				{
					cout << "Giovanni: Fwahahaha! This is my hideout! \nGiovanni: I planned to resurrect Team Rocket here! \nGiovanni: But you have caught me agian! So be it. \nGiovanni: This time, I'm not holding back!\nGiovanni: One more, you shall face Giovanni, the greatest trainer!\n" << endl;
					
					if 	(trainerBattle("Leader", "Giovanni", 42, 0, 2, 53, 111, 31, 34, 112))
					{
						cout << "Giovanni: Ha! That was truly an intense fight!\nGiovanni: You have won. As proof, here is the Earth Badge!" << endl;
						cout << pName << " has received the Earth Badge! " << endl;
						cout << "Giovanni: The Earth Badge makes pokemon of any level obey.\nGiovanni: It is evidence of your mastery as a pokemon trainer!\nGiovanni: With it, you can enter the Pokemon League!\nGiovanni: It is my gift for your Pokemon League challenge." << endl;
						cout << "Giovanni: Having lost, I cannot face my underlings! \nGiovanni: Team Rocket is finished forever! \nGiovanni: I will dedicate my life to the study of Pokemon!\nGiovanni: Let us meet again someday. Farewell!\n" << endl;
						badge[8] = 1;
						badge[0] += 1;
						break;	
					}
		...
		case 6:
			if (x == 7 && badge[3] != 1) // vermil
			{
				...
				if (win == 1)
				{
					...
					if (win == 1)
						cout << "Disciple: Semper fi!" << endl;
				}
				
				if (win == 1)
				{
					cout << "Lt. Surge: Hey kid! What do you think you're doing in here?\nLt. Surge: You won't live long in combat, that's for sure!\nLt. Surge: I tell you kid, electric pokemon saved me during the war!\nLt. Surge: They zapped my enemies into paralysis!\nLt. Surge: The same I'll do to you!\n" << endl;
					if 	(trainerBattle("Leader", "Lt. Surge", 18, 0, 3, 25, 100, 26))
					{
						cout << "Lt. Surge: Whoa! You're the real deal, kid!\nLt. Surge: Fine then, take the Thunder Badge!" << endl;
						cout << pName << " received the Thunder Badge!" << endl;
						cout << "Lt. Surge: A little word of advice, kid! \nLt. Surge: Electricity sure is powerful! \nLt. Surge: But it's useless against ground pokemon!\n" << endl;
						badge[1] = 1;
						badge[0] += 1;
						break;	
					}
		...
		case 8:
			if (x==6 && badge[5] != 1) // fuschia
			{
				...
				if (win == 1)
				{
					...
					if (win == 1)
						cout << "Disciple: Maybe piracy is a better career path. . ." << endl;
				}
				
				if (win == 1)
				{
					cout << "Koga: Fwahahaha! \nKoga: A mere child like you dares to challenge me? \nKoga: Very well, I shall show you the true terror as a ninja master! \nKoga: You shall feel the despair of poison and sleep techniques! \n" << endl; 
					if 	(trainerBattle("Leader", "Koga", 37, 0, 2, 109, 89, 109, 110))
					{
						cout << "Koga: Hmph! You have proven your worth! \nKoga: Here! Take the Soul Badge!" << endl;
		...
		case 10:
			if (x==2 && badge[7] != 1) // cinbar
			{
				...
				if (win == 1)
				{
					cout << "Disciple: They say Blaine survived because of fire pokemon!" << endl;
				...
				if (win == 1)
				{
					cout << "Disciple: I love playing with fire and wetting the bed!" << endl;
				...
				if (win ==1)
				{
					cout << "Blaine: Hah! \nBlaine: I am Blaine! I am the leader of Cinnibar Gym!\nBlaine: My fiery pokemon will incinerate all challengers! \nBlaine: Hah! You better have burn heal!\n" << endl;
					if 	(trainerBattle("Leader", "Blaine", 42, 0, 2, 58, 77, 78, 43))
					{
						cout << "Blaine: I have burnt out!\nBlaine: You have earned the Volcano Badge!" << endl;
		...
		switch (choice)
		{
			case 1:
				...
				cout << "Deposit which pokemon?" << endl;
					...
					for (i=0; party[i][0] != 0; i++)
						cout << "(" << i+1 << "): " << pokedexname[party[i][0]] << " - lvl. " << party[i][1]<< endl;
					...
					cout << "Transferred " << pokedexname[party[--choice][0] ]<< " to the box." << endl;
					int j = choice;
					while (box[j][0] != 0)
						j++;
					for (int k = 0; k < 8 ; k++)
						box[j][k] = party[choice][k];
					for (i=choice; i< 5; i++)
						for (j = 0; j< 8; j++)
			...
			case 2:
				...
				if (party[5][0] !=0)
				{
					...
					for (i=0; box[i][0] != 0; i++)
						cout << "(" << i+1 << "): " << pokedexname[box[i][0]] << " - lvl. " << box[i][1]<< endl;
						cout << "(" << i+1 << "): Go back" << endl;
					...
					cout << "Transferred " << pokedexname[box[--choice][0] ]<< " to the party." << endl;
					int j = choice;
					while (party[j][0] != 0)
						j++;
					for (int k = 0; k < 8 ; k++)
						party[j][k] = box[choice][k];
					for (i=choice; i< 29; i++)
						for (j = 0; j< 8; j++)
							box[i][j] = box[i+1][j];
					for (j = 0; j< 8; j++)	
			...
			case 3:
				{
					...
					cout << "Released " << pokedexname[box[--choice][0] ]<< " back to the wild." << endl;
...
void partyorg()
{
	...
	for (i=0; i < 6; i++)
		if (party[i][0] != 0)
			cout << "(" << i+1 << ") " << setw(10) << pokedexname[party[i][0]] << '\t' << "Lvl " << party[i][1] << '\t' << party[i][2] << "/" << stat("HP", party[i][0], party[i][1]) << endl;
	...
	while (choice != 'q')
	{
		...
		switch(choice)
		{
			...
			case 'p':
				...
				while ((pokemon2 <= 1 && pokemon2 >= 6) || party[pokemon2-1][0] == 0)
					{
						cin.clear();
						cin.ignore(100, '\n');
						cin >> pokemon2;
			...
			case 's':
				cout << '\n' << pokedexname[party[pokemon1][0]]
					 ...
					 << '\n' << "EXP to next level: " << pow(party[pokemon1][1]+1,3)-party[pokemon1][3]
					 << '\n' << "HP: " << party[pokemon1][2] << "/" << stat("HP", party[pokemon1][0], party[pokemon1][1])
					 << '\n' << "Attack: " << stat("A", party[pokemon1][0], party[pokemon1][1])
					 << '\n' << "Defense: " << stat("D", party[pokemon1][0], party[pokemon1][1])
					 << '\n' << "Special Attack: " << stat("SA", party[pokemon1][0], party[pokemon1][1])
					 << '\n' << "Special Defense: " << stat("SD", party[pokemon1][0], party[pokemon1][1])
					 << '\n' << "Speed: " << stat("S", party[pokemon1][0], party[pokemon1][1])
					 << '\n' << "Knows: " << movelist[party[pokemon1][4]] << '\t' << movelist[party[pokemon1][5]] 
					 << '\t' << movelist[party[pokemon1][6]] << '\t' << movelist[party[pokemon1][7]] << endl;
...
{
	int cities[11][2] = {{0,2},{2,2},{2,5},{2,7},{2,10},{5,4},{6,8},{7,2},{7,4},{7,6},{9,4}};
	
	for (int i = 0; i <11; i++)
		if (x==cities[i][0] && (y==cities[i][1]))
			return 1;
	
	return 0;
}

int wildBattle(int level, int levelMod, int pokemon) // battle functino for wild pokemon
{
	if (levelMod > 0)
		level += rand()%levelMod;
	...
	while(party[i][2] != 0)	
		while (party[i][2] != 0)
		{
			...
			cout << "\n\t\t\t\t" << tCurHP << "/" << stat("HP", pokemon, level) << '\t' << "lv. " << level << " " << pokedexname[pokemon] << endl<< endl;	
			// enemy poke data
					
			cout << "lv. " << party[i][1] << " " << pokedexname[party[i][0]] << '\t' << party[i][2] << "/" << stat("HP", party[i][0], party[i][1]) << endl;
			// your poke data
			
			cout << "Use " << movelist[party[i][4]] << " (1), " << movelist[party[i][5]] << " (2), " 
					<< movelist[party[i][6]] << " (3), " << movelist[party[i][7]] << " (4), Throw ball (5), Run (6), Change pokemon (7): ";
				...
				else if(choice == 6) // run check
				{
					...
					else if (stat("S", party[i][0], party[i][1])/stat("S", pokemon, level) > (rand()%100)/100)
					{
						cout << "Got away safely. . . " << endl;
						return 1;
					}
				...
				else if (choice == 7)
				{
					...
					for (int j=0; j < 6; j++)
						if (party[j][0] != 0 && party [j][0] != party[i][0])
							cout << "(" << j+1 << ") " << setw(10) << pokedexname[party[j][0]] << '\t' << "Lvl " << party[j][1] << '\t' << party[j][2] << "/" << stat("HP", party[j][0], party[j][1]) << endl;
				...
				turnResult(tCurHP, party[i][2], pokemon, party[i][0], level, party[i][1], moveselect(pokemon, level, party[i][0]), choice);	
			...
			{
				cout << "Oh no! " << pName << "'s " << pokedexname[party[i][0]] << " fainted!" << endl;
				...
				{	
					...
					{
						if (party[k][2] > 0)
							cout << "(" << k+1 << ") " << setw(10) << pokedexname[party[k][0]] << '\t' << "Lvl " << party[k][1] << '\t' << party[k][2] << "/" << stat("HP", party[k][0], party[k][1]) << endl;
				...
				{
					cout << pName << " has been defeated!\n" << pName << " blacked out. . ." << endl << endl;	
					return 0;
				}
				i=1;
				while ((i <= 1 && i >= 6) || party[i-1][0] == 0 || party[i-1][2] == 0)
					{
						cin >> i;
						while(cin.fail())
							{
				...
				cout << pName << " sent out " << pokedexname[party[--i][0]] << "!" << endl << endl;
				// send out choice
			}
		}	
...
int wildpoke(int x, int y) // randoming which wild pokemon per area
{
	...
	for (i=0; i<10; i++)
		for (j=0; j<11; j++)
			for(k=0; k<4; k++)	
				pokemon[i][j][k] = temppoke[tempmap[i][j]][k];
...
{
	if (rand()%255 < 80)
		if ((maxhp*255*4)/hp >= rand()%255)
			return 1;
	
	return 0;
}

int trainerBattle(string job, string name, int templevel, int levelMod, int SLMod, int pokemon0, int pokemon1, int pokemon2, int pokemon3, int pokemon4, int pokemon5)
{
	int level = templevel;
	if (levelMod > 0)
		level += rand()%levelMod;
	...
	cout << endl << job << " " << name << " has challenged you to a battle!" << endl;
	cout << job << " " << name << " sent out " << pokedexname[pokemon0] << "!" << endl;
	...
	while(party[i][2] != 0)	
		while (epoke[j] > 0 && party[i][2] != 0)
		{
			...
			cout << "\n\t\t\t\t" << tCurHP << "/" << stat("HP", epoke[j], level) << '\t' << "lv. " << level << " " << pokedexname[epoke[j]] << endl<< endl;	
			// enemy poke data
			...
			cout << "Use " << movelist[party[i][4]] << " (1), " << movelist[party[i][5]] << " (2), " 
					<< movelist[party[i][6]] << " (3), " << movelist[party[i][7]] << " (4), Change pokemon (5): ";
				cin >> choice;
				...
				turnResult(tCurHP, party[i][2], epoke[j], party[i][0], level, party[i][1], moveselect(epoke[j], level, party[i][0]), choice);	
				
			if (tCurHP == 0)
			{
				cout << job << " " << name << "'s " << pokedexname[epoke[j]] << " fainted!" << endl;
				xp(1.5, epoke[j], level, party[i][0], party[i][3], party[i][1], i);
				
				
					if (epoke[++j] == 0)
					{
						cout << endl <<  job << " " << name << " has been defeated!" << endl << endl;
						return 1;					
					}
					else
					{
						cout << job << " " << name << " sent out " << pokedexname[epoke[j]] << endl;
						if (levelMod > 0)
							level = templevel + rand()%levelMod;
						level += SLMod;
...
void xp(double trainer, int epoke, int elvl, int ppoke, int& pexp, int& plvl, int partypos) // calcs xp gain, move learning, evolution
{
	int i, basesum=0;
	
	for (i=0; i<6; i++)
		basesum+=pokestat[epoke][i];
	...
	cout << pokedexname[ppoke] << " gained " << dexp << " experience points." << endl;
	
//	cout << static_cast<int>(pow(tempxp,(1/3))) << " " << pow(tempxp,(1/3)) << " " << cbrt(tempxp) << endl;
	...
	while (static_cast<int>(cbrt(tempxp)) > plvl)
		{
			...
			if (plvl >= pokestat[ppoke][6])
			{
				...
				cout << ". . . " << pokedexname[ppoke] << " evolved into " << pokedexname[++party[partypos][0]] << "!!!" << endl << endl;
				ppoke++;
			}
			if (moveset[ppoke][plvl+1] !=  0)
			{
				for (i=4; i < 8; i++)
					if (party[partypos][i] ==0)
					{
						...
						cout << pokedexname[ppoke] << " learned " << movelist[party[partypos][i]] << "!" << endl;
				...
				if (i==8)
					{
						int choice;
						cout <<	pokedexname[ppoke] << " would like to learn " << movelist[moveset[ppoke][plvl+1]]<< ", but already knows 4 moves." << endl;
						cout << "Which move should be deleted?"<<endl;
						cout << movelist[party[partypos][4]-1] << " (1), " << movelist[party[partypos][5]-1] << " (2), " 
							<< movelist[party[partypos][6]-1] << " (3), " << movelist[party[partypos][7]-1] << " (4): ";
						cin >> choice;
						while(cin.fail())
					{
						cin.clear();
						...
						cout << pokedexname[ppoke] << " forgot " << party[partypos][choice+3] << ", and learned " << movelist[moveset[ppoke][plvl]-1]<< endl;
					}
			}
		}
...
void turnResult(int& eHP, int& pHP, int epoke, int ppoke, int elvl, int plvl, int emove, int pmove) // negotiates turns
{
	int damage;
	...
	if (stat("S", ppoke, plvl) >= stat("S", epoke, elvl))
	{
		if (pmove != 0)
		{
			...
			{
				//player first
				if (movestats[pmove][0] == 1 || movestats[pmove][0] == 2 || movestats[pmove][0] == 3 || movestats[pmove][0] == 4 || movestats[pmove][0] == 5 || movestats[pmove][0] == 10)
				{
				...
				if (movestats[pmove][0] == pokestat[ppoke][8] || movestats[pmove][0] == pokestat[ppoke][9])
					stab = 1.5;
		...
		{
			if (movestats[emove][0] == 1 || movestats[emove][0] == 2 || movestats[emove][0] == 3 || movestats[emove][0] == 4 || movestats[emove][0] == 5 || movestats[emove][0] == 10)
			{
			...
			if (movestats[emove][0] == pokestat[epoke][8] || movestats[emove][0] == pokestat[epoke][9])
				stab = 1.5;
...
{
	return typechart[movestats[move][0]][pokestat[pokemon][7]]*typechart[movestats[move][0]][pokestat[pokemon][8]];	
}

int moveselect(int enemypokemon, int enemylvl, int pPokemon) // enemy AI
{
	int templvl = enemylvl, move[4] = {moves(enemypokemon, templvl), moves(enemypokemon, templvl), moves(enemypokemon, templvl), moves(enemypokemon, templvl)};
...
int stat(string stat, int pokemon, int level)  // stat calculator
{
	if (pokemon == 0)
		return 0;
	if (stat == "HP")
		return pokestat[pokemon][0]*2*level/100+level+10;
	if (stat == "A")
		return pokestat[pokemon][1]*2*level/100+5;
	if (stat == "D")
		return pokestat[pokemon][2]*2*level/100+5;
	if (stat == "SA")
		return pokestat[pokemon][3]*2*level/100+5;
	if (stat == "SD")
		return pokestat[pokemon][4]*2*level/100+5;
	if (stat == "S")
		return pokestat[pokemon][5]*2*level/100+5;  
}
...
void pokeget (int pokemon, int level, int hp) // puts poke in box or bag
{
	...
	if (i > 5)
	{
		...
		if (i > 29)
		{
			cout << "Oh no, the box is full! Please withraw or release a pokemon" << endl;
			return;
		}
...
void loadfile() // loads global vars
{
   ...
   // cout << '\n';
	...
	movelistloader.open("movelistnames.txt");
		for (i=0; i<166; i++)
			getline(movelistloader, movelist[i]);
	movelistloader.close();
	
	movelistloader.open("movestats.txt");
		for (i=0; i<166; i++)
			for (j=0; j<3; j++)
				movelistloader >> movestats[i+1][j];
	movelistloader.close();
	
	chartloader.open("typechart.txt");
		for (i=0; i<16; i++)
			for (j=0; j<16; j++)
				chartloader >> typechart[i][j];
	chartloader.close();
	...
	trainerloader.open("jobs.txt");
		for (i=0; i<30; i++)
			trainerloader >> trainers[i][0];
	trainerloader.close();
	
	trainerloader.open("names.txt");
		for (i=0; i<30; i++)
			trainerloader >> trainers[i][1];
	trainerloader.close();
...
void savefile() // saves save file, box, and party
{
	...
	for (j = 0; j<8; j++) // row dex#, level, exp, hp, move1, move2, move3, move4
	{
	...
	save.open("save.txt"); // saves newgame flag, gender, name, coordinates, badges to save.txt
	save << newgame << '\n' << gender << '\n' << pName << '\n' << x << '\n' << y << '\n' << garystarter << endl; 
...
void gamestart() // pokeselect and first battle
{
		newgame = 1;
			...
			while(1)
			{
				...
				{
					case 'w':
						cout << "Oak: You can't go that way! It's dangerous without your own pokemon. Follow me" << endl;
						break;
					case 's':
						cout << "It's the sea. ";
						continue;						
			...
			cout << "Oak takes you to his lab and offers you 3 pokemon and a pokedex." << endl;
			...
			cout << "Oak's grandson barges in, snatches a remaining pokeball, and challenges you to a battle!" << endl;
			...
			cout << "Oak: That was dramatic. Here is your pokedex. Go off now and explore the world of Pokemon!" << endl << endl;
